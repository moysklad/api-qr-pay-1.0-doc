...
Мы предоставляем публичный **API QRPay**, которое позволяет реализовать второй вариант оплаты по QR. Через API
веб-приложение МойСклад сможет запрашивать данные для оплаты у решения. Решение будет обращаться в платёжный сервис
через его API. Одновременно может быть подключено только одно решение оплаты по QR.
